...
import { SentenceTile } from "./sentence-tile";
import { SentenceDropZone } from "./sentence-drop-zone";

import type {
  SentenceOrderPayload,
  SentenceTile as Tile,
...
export function SentenceOrderGame({
  ...
  async function submit() {
    ...
    const gameResult = {
      answer: result,
      isCorrect: result.toLowerCase() === payload.sentence.toLowerCase(),
    };
    if (!gameResult.isCorrect)
      toast.error("Peccato... La frase non e' corretta");
    else toast("Bravo!!! la frase e' corretta");
  ...
  function addWord(buffer: string) {
    const tile = availableTiles.find(
      (t) => t.word.toLowerCase() === buffer.toLowerCase(),
    );
    if (!tile) return;
  ...
  }, [inputBuffer, availableTiles, answerIds]);
    ...
    <Card className="mx-auto max-w-4xl">
      ...
      <CardContent className="space-y-8">
        ...
        <div className="flex justify-center gap-4">
          ...
          <Button disabled={availableTiles.length >= 2} onClick={submit}>
